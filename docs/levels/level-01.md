...
며칠 지나니 기획자가 기능을 하나씩 들고 와요. 받은 말을 그대로 옮기면 이래요.

- "정적 파일은 `/static/`으로 시작하는 경로면 전부 정적 핸들러로 내려가게 해주세요."
- "관리자 화면은 `/admin/*` 경로에서 처리되게 해주세요. 그 아래 한 칸은 뭐가 오든 관리자고요."
- "예약 기능도 곧 붙어요. `/reservations/*`, `/rooms/*`, `/payments/*` ... 이렇게 점점 늘 텐데 알아서 잘 받아 주세요. 아, 나중엔 정규식으로도 매칭하고 싶을 수 있어요."

요청들을 가만히 들여다보면, 핸들러를 고르는 일보다 "경로가 맞는지 따지는 방식"이 제각각이라는 게 먼저 눈에 들어와요. 어떤 건 글자로 시작하는지, 어떤 건 한 칸을 비워 두는지, 어떤 건 모양 전체를 보는지요.

일단 시키는 대로, 지금의 `getHandler`에 그대로 욱여넣어 보세요. 십중팔구 저 `if (path.equals(...))` 한 줄이 이렇게 부풀어 오릅니다.
...
after 커밋과 회고까지 마쳤나요? 그럼 이제 당신이 다시 발견한 구조가 무엇이었는지 확인할 차례예요. [정답과 이정표](answers/level-01.md)를 열어, refactoring.guru의 카탈로그와 당신의 코드를 나란히 놓고 대조해 보세요. 이름이 같아도 당신 버전과 다른 점이 분명 있을 거예요. 그 차이를 회고에 한 줄 보태 두면 그게 진짜 재발견입니다.

아직 before -> 고통 -> after를 다 거치지 않았다면 지금은 열지 마세요. 이름을 미리 알면 "이 변경 압력을 어떻게 풀까"라는 고민이 "그 패턴을 어떻게 쓸까"로 바뀌어, 이 단계에서 얻을 게 절반으로 줄거든요.

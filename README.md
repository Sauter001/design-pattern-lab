...
- before/after는 격리 폴더 말고 git 커밋으로 남겨요. 권장 리듬은 이래요.
  ...
  3. 리팩토링(패턴 도입) 커밋
  이렇게 쌓이면 커밋 로그 자체가 "이 패턴이 왜 들어왔나"의 서사가 돼요.
...
아직 뼈대만 있어요. 각 모듈엔 층 책임이랑 등장 예정 패턴을 적은 `package-info.java` 하나, 빌드 배선을 확인하는 스모크 테스트 하나가 다예요. 패턴 구현은 비어 있어요. 그건 본인이 채울 거고요.
